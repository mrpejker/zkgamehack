 ...
 */
export class ReductionTaskFlow {
    constructor(options, flowCreator) {
    ...
    resolveReducibleTasks(pendingInputs, reducible) {
        ...
        for (const [index, first] of remainingInputs.entries()) {
            const secondIndex = remainingInputs.findIndex((second, index2) => index2 > index &&
                (reducible(first, second) || reducible(second, first)));
            if (secondIndex > 0) {
                const r2 = remainingInputs[secondIndex];
                remainingInputs = remainingInputs.filter((unused, index2) => index2 !== index && index2 !== secondIndex);
        ...
        if (this.flow.tasksInProgress === 0 &&
            res.length * 2 < queueSize &&
            this.flow.state.numMergesCompleted + res.length * 2 + queueSize ===
                this.options.inputLength) {
            log.error(`Flow ${this.flow.flowId} seems to have halted with ${this.flow.state.queue.length} elements left in the queue`);
        }
        return { availableReductions: res, touchedIndizes };
    }
    async resolveReduction() {
        const { flow, options } = this;
        if (options.inputLength - flow.state.numMergesCompleted === 1 &&
        ...
        if (flow.state.queue.length >= 2) {
            const { availableReductions, touchedIndizes } = this.resolveReducibleTasks(flow.state.queue, options.mergableFunction);
            ...
            await flow.forEach(availableReductions, async (reduction) => {
                const taskParameters = [reduction.r1, reduction.r2];
                await flow.pushTask(options.reductionTask, taskParameters, async (result) => {
                    flow.state.queue.push(result);
                    flow.state.numMergesCompleted += 1;
                    await this.resolveReduction();
                });
            });
            flow.state.queue = flow.state.queue.filter((ignored, index) => !touchedIndizes.includes(index));
        }
    }
    async initCompletionCallback(callback) {
        if (this.started) {
            throw new Error("Flow already started, use pushInput() to add inputs");
        }
        this.started = true;
        try {
            const result = await this.flow.withFlow(async () => { });
    ...
    async execute(inputs = []) {
        ...
        this.started = true;
        return await this.flow.withFlow(async () => {
    ...
    async pushInput(input) {
        await this.flow.pushTask(this.options.mappingTask, input, async (result) => {
