...
async function buildAppStaticPaths({ dir, page, distDir, configFileName, generateParams, isrFlushToDisk, cacheHandler, requestHeaders, maxMemoryCacheSize, fetchCacheKeyPrefix, nextConfigOutput, ComponentMod, isRoutePPREnabled, isAppPPRFallbacksEnabled }) {
    ...
    return (0, _withstaticgenerationstore.withStaticGenerationStore)(ComponentMod.staticGenerationAsyncStorage, {
        ...
        if (typeof (pageEntry == null ? void 0 : pageEntry.getStaticPaths) === 'function') {
            ...
            const buildParams = async (paramsItems = [
                ...
                if (typeof current.generateStaticParams !== 'function' && idx < generateParams.length) {
                ...
                if (current.generateStaticParams) {
                    ...
                    if (store) {
                        var _current_config, _current_config1, _current_config2;
                        if (typeof (current == null ? void 0 : (_current_config = current.config) == null ? void 0 : _current_config.fetchCache) !== 'undefined') {
                            store.fetchCache = current.config.fetchCache;
                        }
                        if (typeof (current == null ? void 0 : (_current_config1 = current.config) == null ? void 0 : _current_config1.revalidate) !== 'undefined') {
                            store.revalidate = current.config.revalidate;
                        }
                        if ((current == null ? void 0 : (_current_config2 = current.config) == null ? void 0 : _current_config2.dynamic) === 'force-dynamic') {
            ...
            if (generateParams.some((generate)=>{
            ...
            const dynamicParams = generateParams.every((param)=>{
                var _param_config;
                return ((_param_config = param.config) == null ? void 0 : _param_config.dynamicParams) !== false;
            });
            ...
            const fallbackMode = dynamicParams ? supportsStaticGeneration ? supportsPPRFallbacks ? _fallback.FallbackMode.PRERENDER : _fallback.FallbackMode.BLOCKING_STATIC_RENDER : undefined : _fallback.FallbackMode.NOT_FOUND;
            let result = {
                fallbackMode,
...
async function isPageStatic({ dir, page, distDir, configFileName, runtimeEnvConfig, httpAgentOptions, locales, defaultLocale, parentId, pageRuntime, edgeInfo, pageType, originalAppPath, isrFlushToDisk, maxMemoryCacheSize, nextConfigOutput, cacheHandler, pprConfig, isAppPPRFallbacksEnabled }) {
...
function reduceAppConfig(segments) {
    ...
    for (const segment of segments){
        ...
        const { dynamic, fetchCache, preferredRegion, revalidate, experimental_ppr } = segment.config;
